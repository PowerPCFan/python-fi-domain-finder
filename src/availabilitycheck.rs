...
pub async fn check_domain_available(client: &reqwest::Client, domain: &str) {
    ...
    form_data.insert("searched_domain", domain);
    ...
    let res = client.post("https://www.domainhotelli.fi/asiakkaat/modules/addons/ispapidomaincheck/domain.php")
        .form(&form_data)
        .send()
        .await;
    ...
    let available = body.get("data").unwrap()[0].get("status").unwrap() == "available";
